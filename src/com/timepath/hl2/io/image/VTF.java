...
public class VTF implements ViewableData {
    ...
    private static final int    VTF_RSRC_TEXTURE_CRC = 'C' | ( 'R' << 8 ) | ( 'C' << 16 ) | ( '\2' << 24 );
    ...
    public static VTF load(InputStream is) throws IOException {
        VTF vtf = new VTF(); if(vtf.loadFromStream(is)) {
            return vtf;
        } return null;
    }

    boolean loadFromStream(InputStream is) throws IOException {
        int magic = is.read() | ( is.read() << 8 ) | ( is.read() << 16 ) | ( is.read() << 24 ); if(magic != HEADER) {
            LOG.log(Level.FINE, "Invalid VTF file: {0}", magic); return false;
        } byte[] array = new byte[4 + is.available()]; is.read(array, 4, array.length - 4); buf = ByteBuffer.wrap(array);
        buf.order(ByteOrder.LITTLE_ENDIAN); buf.position(4); version = new int[] { buf.getInt(), buf.getInt() };
        headerSize = buf.getInt(); width = buf.getShort(); height = buf.getShort(); flags = buf.getInt();
        EnumSet<CompiledVtfFlags> c = EnumFlags.decode(flags, CompiledVtfFlags.class); frameCount = buf.getShort();
        frameFirst = buf.getShort(); buf.get(new byte[4]);
        reflectivity = new float[] { buf.getFloat(), buf.getFloat(), buf.getFloat() }; buf.get(new byte[4]);
        bumpScale = buf.getFloat(); format = ImageFormat.getEnumForIndex(buf.getInt()); mipCount = buf.get();
        thumbFormat = ImageFormat.getEnumForIndex(buf.getInt()); thumbWidth = buf.get(); thumbHeight = buf.get();
        depth = buf.getShort(); Object[][] debug = {
                { "Width = ", width },
                { "Height = ", height },
                { "Frames = ", frameCount },
                { "Flags = ", c },
                { "Format = ", format },
                { "MipCount = ", mipCount },
        }; LOG.fine(StringUtils.fromDoubleArray(debug, "VTF:")); return true;
    }
    ...
    public void getControls() {
        ...
        // until here are for
        int crcHead = buf.getInt(); int crc = buf.getInt(); if(crcHead != VTF_RSRC_TEXTURE_CRC) {
    ...
    public Image getThumbImage() {
        if(thumbImage == null) {
            buf.position(headerSize); byte[] thumbData = new byte[Math.max(thumbWidth, 4) * Math.max(thumbHeight, 4) /
                                                                  2]; // DXT1. Each 'block' is 4*4 pixels. 16 pixels become 8
                                                                  // bytes
            buf.get(thumbData); thumbImage = DXTLoader.loadDXT1(thumbData, thumbWidth, thumbHeight);
        } return thumbImage;
    }
    ...
    public Image getImage(int level, int frame) {
        ...
        if(thumbWidth == 0 || thumbHeight == 0) {
            thumbLen = 0;
        } buf.position(headerSize + thumbLen); BufferedImage image = null;
        int[] sizesX = new int[mipCount]; // smallest -> largest {1, 2, 4, 8, 16, 32, 64, 128}
        int[] sizesY = new int[mipCount]; for(int n = 0; n < mipCount; n++) {
            sizesX[mipCount - 1 - n] = Math.max(width >>> n, 1); sizesY[mipCount - 1 - n] = Math.max(height >>> n, 1);
        } for(int i = 0; i < mipCount; i++) {
            int w = sizesX[i]; int h = sizesY[i]; LOG.log(Level.FINE, "{0}, {1}", new Object[] { w, h });
            int nBytes = format.getBytes(w, h); if(i == mipCount - level - 1) {
                byte[] imageData = new byte[nBytes * frameCount]; try {
                    buf.get(imageData);
                } catch(BufferUnderflowException e) {
                    LOG.log(Level.SEVERE, "Underflow; {0}", nBytes);
                } System.arraycopy(imageData, frame * nBytes, imageData, 0, nBytes);
                LOG.log(Level.INFO, "VTF format {0}", format); image = new BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB);
                Graphics2D g = (Graphics2D) image.getGraphics(); g.drawImage(format.load(imageData, w, h), 0, 0, w, h, null);
            } else {
