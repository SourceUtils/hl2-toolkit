...
public class Message {
    ...
    Message(ByteBuffer buffer, HL2DEM outer) {
        this.outer = outer; int b = buffer.get(); type = MessageType.get(b); if(type == null) {
            LOG.log(Level.SEVERE, "Unknown demo message type encountered: {0}", b);
        } tick = buffer.getShort() + ( buffer.get() << 16 ); if(type != MessageType.Stop) {
            buffer.get();
        } LOG.log(Level.FINE,
    ...
    void parse() {
        if(data == null) {
            ...
            case Signon: case Packet: {
                BitBuffer bb = new BitBuffer(data); List<Pair<Object, Object>> values = new LinkedList<>(); String error = null;
                while(bb.remaining() >= 1) {
                    try {
                        int mid = (int) bb.getBits(outer.header.networkProtocol >= 16 ? 6 : 5); Packet p = Packet.get(mid);
                        if(p != null) {
                            List<Pair<Object, Object>> list = new LinkedList<>(); try {
                        ...
                        incomplete = true; values.add(new Pair<Object, Object>("error", error)); LOG.log(Level.WARNING, error);
                        break;
                    }
                } meta.add(new Pair<Object, Object>(this, values));
            } break; case ConsoleCmd: {
                ByteBuffer b = data; Level l = Level.FINE; String cmd = DataUtils.getText(b).trim();
                meta.add(new Pair<Object, Object>(this, cmd)); if(b.remaining() > 0) {
                ...
                BitBuffer bb = new BitBuffer(data); if(bb.getBoolean()) {
            ...
            // TODO
            case DataTables: case StringTables: break;
        }
    }
}
