...
public class RES extends VDF1 implements Savable {
    ...
    private static void clientScheme(DefaultMutableTreeNode props) {
        LOG.info("Found clientscheme"); if(!( props instanceof VDFNode1 )) {
            LOG.log(Level.WARNING, "TreeNode not instanceof VDFNode", props); return;
        } VDFNode1 root = ( (VDFNode1) props ).get("Scheme").get("Fonts"); for(int i = 0; i < root.getChildCount(); i++) {
            TreeNode node = root.getChildAt(i); if(!( node instanceof VDFNode1 )) {
                continue;
            } VDFNode1 fontNode = (VDFNode1) node; VDFNode1 detailNode = null; for(int j = 0; j < fontNode.getChildCount(); j++) {
                TreeNode detail = fontNode.getChildAt(j); if(!( detail instanceof VDFNode1 )) {
                    continue;
                } detailNode = (VDFNode1) detail; break; // XXX: hardcoded detail level (the first one)
            } if(detailNode == null) {
                continue;
