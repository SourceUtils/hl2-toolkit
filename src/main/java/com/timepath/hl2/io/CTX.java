...
public class CTX {
    ...
    public static void main(String... args) {
        try {
            @NotNull InputStream is = new FileInputStream(args[0]);
            String key = args[1];
            if (key == null) {
                key = TF2;
            }
            @NotNull InputStream de = decrypt(key.getBytes("UTF-8"), is);
            @NotNull InputStream is2 = encrypt(key.getBytes("UTF-8"), de);
            is = new FileInputStream(args[0]);
            de = decrypt(key.getBytes("UTF-8"), is);
            is = new FileInputStream(args[0]);
            int bs = 4096;
            ByteBuffer debuf = ByteBuffer.allocate(de.available());
            @NotNull byte[] inde = new byte[bs];
    ...
    @NotNull
    private static InputStream encrypt(byte[] key, @NotNull InputStream is) throws IOException {
        return new ByteArrayInputStream(method(key, is, false));
    }

    @NotNull
    public static InputStream decrypt(byte[] key, @NotNull InputStream is) throws IOException {
        return new ByteArrayInputStream(method(key, is, true));
    }

    @NotNull
    private static byte[] method(byte[] key, @NotNull InputStream is, boolean decrypt) throws IOException {
        ByteBuffer buf = ByteBuffer.allocate(is.available());
        @NotNull IceKey ice = new IceKey(0);
        ice.set(key);
        int bs = 8; // ice.blockSize();
        @NotNull byte[] in = new byte[bs];
        @NotNull byte[] out = new byte[bs];
        int prevRead = 0;
        for (int read = 0; read != -1; read = is.read(in, 0, bs)) {
            prevRead = read;
            //            if (read != bs) {
