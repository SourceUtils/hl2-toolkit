...
public enum class LumpType(val ID: Int, val handler: LumpHandler<*>? = null) {
    /** Map entities */
    LUMP_ENTITIES(0, Entities.Handler()),
    /** Plane array */
    LUMP_PLANES(1, Plane.Handler()),
    /** Index to texture names */
    LUMP_TEXDATA(2),
    /** Vertex array */
    LUMP_VERTEXES(3, Vertex.Hander()),
    ...
    LUMP_FACEIDS(11),
    /** Edge array */
    LUMP_EDGES(12, Edge.Handler()),
    /** Index of edges */
    LUMP_SURFEDGES(13, SurfaceEdge.Handler()),
    ...
    LUMP_PAKFILE(40, Pakfile.Handler()),
    ...
    public fun <T : Any> handle(l: Lump, ois: OrderedInputStream): T? {
        if (handler == null) {
            LOG.warning { "No handler for ${this}" }
            return null
        }
        return handler.invoke(l, ois) as T
    }
}
